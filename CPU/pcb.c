...
int tamanioTotalPCB(t_PCB* pcb){
	...
	return sizeof(t_pid) + sizeof(t_num) * 8 +
...
void *serializarPCB(t_PCB* pcb){
	...
	memcpy(buffer + offset, &pcb->sp, tmpsize = sizeof(t_num));
		offset += tmpsize;
	memcpy(buffer + offset, &pcb->cantRafagas, tmpsize = sizeof(t_num));
		offset += tmpsize;
...
t_PCB *desserealizarPCB(void* buffer){
	...
	memcpy(&pcb->pid, buffer + offset, tmpsize = sizeof(t_pid));
		offset += tmpsize;
	...
	memcpy(&pcb->sp, buffer + offset, tmpsize = sizeof(t_num));
		offset += tmpsize;
	memcpy(&pcb->cantRafagas, buffer + offset, tmpsize = sizeof(t_num));
		offset += tmpsize;
